...
const TaskCard = ({
  ...
  const approvalPending =
    (task?.status || status) === "Pending Approval" &&
  ...
  return (
    ...
    >
      ...
      {approvalPending && (typeof onApprove === "function" || typeof onReject === "function") ? (
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {typeof onReject === "function" ? (
            <button
              type="button"
              className="inline-flex items-center justify-center rounded-full border border-rose-200 bg-white px-4 py-2 text-xs font-semibold text-rose-700 shadow-sm transition hover:border-rose-300 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60"
              onClick={(event) => {
                event.stopPropagation();
                onReject(task);
              }}
